...
timePicker.onclick = function(){
    ...
    var speedConstrain = 0+"< value and value <" + 5000 + "and topicid in (6,15,24)";
    getTimelineData(startTime, endTime,speedConstrain);
}

var width = 500;
  
function timelineRect(timelineData) {
    var chart = d3.timeline().click(function (d, i, datum) {
    alert(datum.label);
    });
...
function getTimelineData(startTime, endTime,constrain){
    ...
    .then(function (data) {
        ...
        var timelineData2=[{times:[]},{times:[]},{times:[]}]
        
        for(var i=1; i<= timelineData.length; i++){
            var groupNumber=timelineData[i-1].group
            console.log(timelineData2[groupNumber-1].times)
            timelineData2[groupNumber-1].times.push({
                "starting_time": Math.round(new Date(timelineData[i-1].start).getTime()/1000),
                "ending_time": Math.round(new Date(timelineData[i-1].end).getTime()/1000),
            })
        }

        var timelineData3 = []
        for(var i=0; i<3;i++){
            if(timelineData2[i].times.length != 0){
                timelineData3.push(timelineData2[i])
            }
        }

        console.log(timelineData3)

        timelineRect(timelineData3)
